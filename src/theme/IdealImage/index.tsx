/**
 * Theme override to preserve passing standard img props (alt/className/style)
 * through to the underlying IdealImageLegacy implementation.
 *
 * In newer plugin versions, these props are not forwarded, which breaks MDX
 * usage like: <Photo className="landing-logo" ... />.
 */
import React, {type ReactNode} from 'react';
import {translate} from '@docusaurus/Translate';
import ReactIdealImage, {type IconKey, type State} from '@theme/IdealImageLegacy';
...
export default function IdealImage(props: Props): ReactNode {
  ...
  // Dev env: regular img keeps MDX props working
  if (typeof img === 'string' || 'default' in img) {
    // eslint-disable-next-line jsx-a11y/alt-text
    return <img src={typeof img === 'string' ? img : img.default} {...propsRest} />;
  }

  // Prod build: forward standard props (className/alt/style/...) to legacy impl
  return (
    <ReactIdealImage
      {...propsRest}
