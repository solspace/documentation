...
function SearchPageContent(): JSX.Element {
  ...
  const makeSearch = useEvent((page: number = 0) => {
    if (contextualSearch) {
      ...
      if (mainCategory) {
        const categoryData = docsTag[mainCategory];

        if (categoryData) {
          if (selectedVersion) {
            const versionData = categoryData.find(
  ...
  const mappedCategories = useMemo(() => {
    return Object.keys(docsTag).map((slug) => {
      const plugin = allPlugins.find((p) =>
        p.items.some((item) => item.slug === slug)
      );

      const matchingItem = plugin?.items.find((item) => item.slug === slug);

      return {
        key: slug,
        displayName:
          plugin && matchingItem ? `${plugin.name} ${matchingItem.name}` : slug,
      };
    });
  }, [docsTag, allPlugins]);
  ...
  const handleMainCategory = (value: string) => {
    ...
    setVersionOptions(docsTag[value]);

    const categoryData = docsTag[value];

    if (selectedVersion && categoryData) {
      const versionExists = categoryData.some(
        (item) => item.version === selectedVersion
      );
  ...
  useEffect(() => {
    ...
    urlParams.set('plugin', mainCategory);

    if (selectedVersion) {
      urlParams.set('version', selectedVersion);
    } else {
      setVersionOptions(docsTag[mainCategory]);
    }

    history.push({
      search: urlParams.toString(),
    });
  }, [mainCategory, selectedVersion]);
  ...
  useEffect(() => {
    ...
    if (plugin) {
      setMainCategory(plugin);
      setVersionOptions(docsTag[plugin]);
  ...
  return (
    <Layout>
      ...
      <div className="container margin-vert--lg">
        ...
        >
          <div
            className={clsx('col !p-0', styles.searchQueryColumn, {
          ...
          <div className="col col--2 !p-0">
            <label htmlFor="main-category-select">Plugin:</label>
            <select
              id="main-category-select"
              value={mainCategory}
              onChange={(e) => handleMainCategory(e.target.value)}
              className={clsx(styles.searchVersionInput)}
            >
              <option value="" disabled>
                Select a Plugin
              </option>
              {mappedCategories.map(({ key, displayName }) => (
