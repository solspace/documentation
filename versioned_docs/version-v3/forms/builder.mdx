...
The **Form Settings** tab (cog icon) handles the overall settings for the form. It will also display and handle editing of settings for [fields](./fields.mdx) and [multi-page page tabs](./multi-page-forms.mdx).
...
- **Formatting Template** (optional) - if using the [render() method](../templates/queries/form.mdx#render), this essentially allows you to "attach" a formatting template to a form so that you don't need to include formatting inside the template(s) you place the form inside.
  - Select from an existing sample or custom template, or create a new one.
    - If creating a new one, a new template file will automatically be created for you in your [Formatting Template Directory](../configuration/settings.mdx) containing sample template code that you can adjust later.
    - Users/groups need to have permissions access for **Settings** to create new formatting templates.
- **Enable AJAX** - enables Freeform's [built-in automated AJAX submit support](../guides/templating/ajax-forms.mdx). Once enabled, Freeform will handle the rest (as long as you're using standard template formatting - very custom formatting templates may not work properly).
...
- **Store Submitted Data** - allows you to specify whether the submission data for this form should be stored in the database.
  - Typical use-cases for not storing submission data on your site would be if you just wanted the [email notification](./email-notifications.mdx) and/or wanted the data submitted over to an [Email Marketing](../integrations/index.mdx#email-marketing) or [CRM](../integrations/index.mdx#crm) integration.
  - It's important to note that this likely isn't a complete solution for [GDPR Compliance](../configuration/gdpr-compliance.mdx) if that is why you are using this feature. Please read our official [Guide to GDPR Compliance with Freeform](../configuration/gdpr-compliance.mdx) for more information.
- **Opt-In Data Storage Checkbox** - allows the submitter to choose whether or not they want their submission data stored on your site. In the select dropdown, select a **Checkbox** fieldtype you want to be the designated opt-in field, or choose **Disabled** to not use this feature.
  - This setting will still send off data to [integrations](../integrations/index.mdx) and [email notifications](./email-notifications.mdx).
  - It's important to note that this likely isn't a complete solution for [GDPR Compliance](../configuration/gdpr-compliance.mdx) if that is why you are using this feature. Please read our official [Guide to GDPR Compliance with Freeform](../configuration/gdpr-compliance.mdx) for more information.
...
- **POST Forwarding**<Badge type="pro" text="Pro" /> - if you need to have the POST data of this form submitted to an external API, provide that custom URL here.
  ...
  - See [POST Forwarding documentation](../integrations/index.mdx#other) for more information about this feature.
...
Please see the [Element Connections](../integrations/index.mdx#craft-elements) documentation for more information about this feature.

## CRM Integrations<Badge type="pro" text="Pro" />

The **CRM Integrations** tab (cloud icon) is where you configure any [CRM integrations](../integrations/index.mdx#crm) you may have on a per-form level. Simply choose an integration name and then map out your Freeform fields to your CRM's fields. This tab will only show if you have any CRM integrations configured.
...
The [Important Update Notices](../configuration/notices-alerts.mdx#update-notices--announcements) area keeps you informed about issues that may specifically affect your site.
