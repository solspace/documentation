...
description: Manage all Freeform 5.x settings via Craft CMS Project Config, including General Settings, Form Behavior, Spam Protection, Form Builder defaults, and more.
---
...
// highlight-end
    ...
    exportTrackedUrlParameters: true
...
// highlight-end
    ...
    errorNotificationRecipients: bob@acmewidgets.net
    digestFrequency: 2
...
← Include Tracked URL Parameters when Exporting (NEW IN 5.14.16+)
...
← Automatically Purge Submission Data (in days)
← Apply Purge to Forms ID(s) (NEW IN 5.14+)
...
← Prevent Duplicate Submissions (Idempotency Keys) (NEW IN 5.13+)
← CSRF Token Refresh Method. Options are 'once' (recommended), 'always', or 'none'.
← Use Queue for AI Processing (NEW IN 5.13+)
...
← Send Critical System Error Report Immediately (NEW IN 5.13+)
