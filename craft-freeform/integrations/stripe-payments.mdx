...
Stripe fields work with Freeform’s official headless React packages. Configure Stripe in Freeform as usual, enable the form for headless, and pass `recommendedExtensions` (includes `stripePaymentExtension`).

See **[Headless → React JS → Stripe payments](../headless/reactjs.mdx#stripe-payments)** for setup, same-origin proxy notes, multipage tips, and how Payment Element confirmation ties into Freeform’s existing Stripe callback / webhook.
