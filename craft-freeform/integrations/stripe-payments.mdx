...
sidebar_position: 34
description: Stripe Payments integration for Freeform 5.x in Craft CMS.
---
...
<Player
  url={ require('@site/static/videos/craft/freeform/v5/stripe-payments.mp4').default }
...
<VerticalStepWrapper>
  <StepMarkdown stepTitle="Prepare Freeform">
    ...
    - Find _Stripe_ in the list of **service providers**.
  </StepMarkdown>
  ...
  <StepMarkdown stepTitle="Configure the Stripe Webhook">
    ...
    - Configure the webhook:
      - Copy the **Webhook URL** field value in Freeform (e.g. `https://mysite.net/freeform/payments/stripe/webhook`) and paste it into the **Endpoint URL** setting.
...
<VerticalStepWrapper>
  <StepMarkdown stepTitle="Enable Integration">
    ...
    ---
    #### Additional settings
    - **Payment Configuration ID**<Badge type="feature" text="New in 5.11.11+" />
      - To use a Stripe payment configuration other than the Default, enter its ID here. Payment configurations apply to one-time payments only. To limit methods on subscriptions, use the _Subscription Payment Methods_ setting below.
    - **Subscription Payment Methods**<Badge type="feature" text="New in 5.15.29+" />
      - Comma-separated Stripe payment method types allowed on subscriptions (e.g. `card, paypal, bacs_debit, us_bank_account`). Leave blank to use your Stripe account default.
      - See the [full list of Stripe values available](#subscription-payment-methods) for this setting.
...
To use a different Stripe payment configuration other than the Default, enter its ID here. This is a per-form settings.

::: note
Payment configurations apply to one-time payments only. To limit methods on subscriptions, use the _Subscription Payment Methods](#subscription-payment-methods)_ setting below.
:::

#### Subscription Payment Methods<Badge type="feature" text="New in 5.15.29+" />

Unlike single one-time pauments, Subscription payments will always use the Stripe account default due to a limitation with the API. If you wish to use a custom set of payment options for a form accepting Subscription payments, use this setting and enter a comma-separated list of Stripe payment method types (e.g. `card, paypal, bacs_debit, us_bank_account`). Leave blank to use your Stripe account default.

<details className="hidden-heading">
<summary>Full list of Stripe values available</summary>

| Payment type                  | Value              |
| ----------------------------- | ------------------ |
| ACH Debit                     | `ach_debit`        |
| ACSS Debit                    | `acss_debit`       |
| Affirm                        | `affirm`           |
| Alipay                        | `alipay`           |
| Amazon Pay                    | `amazon_pay`       |
| Australian BECS Direct Debit  | `au_becs_debit`    |
...
| Naver Pay                     | `naver_pay`        |
| New Zealand BECS Direct Debit | `nz_bank_account`  |
| Przelewy24                    | `p24`              |
...
- `payments.status`
  - Outputs the payment status to for the submission, e.g. `succeeded`, `active`, `failed`.
...
- `payments.unsubscribeUrl` (subscription only) - Outputs an unsubscribe URL for the user to self-unsubscribe from a subscription, e.g. `https://mysite.net/freeform/payment-subscription/37/cancel/2ba40891782393e3ff1ae9c3b2b6786d5318eabc`.
...
If you wish to make any customization to the handling of Stripe payment fields, you can use the [Template Overrides](../templates/formatting/index.mdx#template-overrides) feature:

<Tabs>
  <TabItem value="render-element-params-form" label="At Form-level" default>
    ```twig
    {{ form.renderTag({
      fields: {
        "@stripe": {
  ...
  <TabItem value="render-element-params-field" label="At Field-level">
    ```twig {2}
    ...
...
Stripe fields work with Freeform’s official headless React and Vue packages. Configure Stripe in Freeform as usual, enable the form for headless, and pass `recommendedExtensions` (includes `stripePaymentExtension`).

See **[Headless → React JS → Stripe payments](../headless/reactjs.mdx#stripe-payments)** or **[Vue.js → Payments](../headless/vuejs.mdx#payments-stripe-square-paypal-mollie)** for setup, same-origin proxy notes, multipage tips, and how Payment Element confirmation ties into Freeform’s existing Stripe callback / webhook.
