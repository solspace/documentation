...
- In the **URL** setting, add your custom action URL there. Freeform will make an extra POST submit to that URL once it has successfully validated the form submission, etc.
- In the **Error Trigger** setting, enter a value like `error` (assuming the error page contains that word, and success wouldn't). When Freeform detects that word (or phrase) in the success/error page from the custom Click POST URL, it'll assume it's an error and then log the error(s) to the Freeform error log.
