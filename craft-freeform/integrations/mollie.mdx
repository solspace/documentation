...
title: 'Mollie'
sidebar_position: 23
description: Mollie integration for Freeform 5.x in Craft CMS.
---
...
# Mollie<Badge type="pro" text="Pro" /><Badge type="feature" text="New in 5.13+" />
...
Mollie fields work with Freeform’s official headless React and Vue packages. Configure Mollie in Freeform as usual, enable the form for headless, and pass `recommendedExtensions` (includes `molliePaymentExtension`).

See **[Headless → React JS → Mollie payments](../headless/reactjs.mdx#mollie-payments)** or **[Vue.js → Payments](../headless/vuejs.mdx#payments-stripe-square-paypal-mollie)** for setup, same-origin proxy notes, multipage tips, and how create → submit → hosted checkout ties into Freeform’s existing Mollie callback / webhook.
