...
Mollie fields work with Freeform’s official headless React packages. Configure Mollie in Freeform as usual, enable the form for headless, and pass `recommendedExtensions` (includes `molliePaymentExtension`).

See **[Headless → React JS → Mollie payments](../headless/reactjs.mdx#mollie-payments)** for setup, same-origin proxy notes, multipage tips, and how create → submit → hosted checkout ties into Freeform’s existing Mollie callback / webhook.
