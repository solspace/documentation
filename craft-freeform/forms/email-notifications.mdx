...
sidebar_position: 3
description: Configure and customize email notifications for Freeform 5.x forms in Craft CMS, including Admin, Conditional, User Select, and Email Fields types.
---
...
Freeform allows you to send email notifications upon submittal of a form. You can choose between form-specific notification templates or use global templates that can be reused for multiple forms, saving you time when you are managing many forms.
...
<Photo
  img={require('@site/static/img/craft/freeform/v5/cp/builder-notification-admin.png')}
  alt="Admin Notifications"
/>
...
<details className="hidden-heading">
  ...
  <VerticalStepWrapper>
    ...
    <StepMarkdown>
      Create a new notification template or choose an existing one from the **Notification Template** setting.
    </StepMarkdown>
    <StepMarkdown>
...
<Photo
  img={require('@site/static/img/craft/freeform/v5/cp/builder-notification-conditional.png')}
  alt="Conditional Notifications"
/>
...
<Photo
  img={require('@site/static/img/craft/freeform/v5/cp/builder-notification-user-select.png')}
  alt="User Select Notifications"
/>
...
<details className="hidden-heading">
  ...
  <VerticalStepWrapper>
    ...
    <StepMarkdown>
      Create a new notification template or choose an existing one from the **Default Notification Template** setting.
    </StepMarkdown>
    <StepMarkdown>
...
<Photo
  img={require('@site/static/img/craft/freeform/v5/cp/builder-notification-email-field.png')}
  alt="Admin Notifications"
/>
...
- **Subject** - the subject line for the email notification.
  - May include any token variables or Freeform field Twig variables (`{{ fieldName }}`) as well as `form.name`.
- **From Email** - the email address the email notification will appear from.
  - May include any token variables or Freeform field Twig variables (`{{ fieldName }}`).
  :::warning
...
- **From Name** - the email address the email notification will appear from.
  - May include any token variables or Freeform field Twig variables (`{{ fieldName }}`), e.g. `{{ firstName }} {{ lastName }}`
  :::warning
  Using a dynamic approach with variables like above could have your emails marked as spam. It would be better to use something general here and set these in the _Reply-to Email_ and _Reply-to Name_ settings instead.
  :::
...
#### Template Wrappers<Badge type="feature" text="New in 5.11+" />

Notification template wrappers can be used to help with setting common template code in notification templates. For example, you might include header and footer HTML code in a template wrapper, and then keep form-specific content inside the regular notification templates. You can then specify a template wrapper in each notification template.

<Tabs>
  <TabItem value="template-wrappers-form" label="Form Builder" default>
    <Photo
      img={require('@site/static/img/craft/freeform/v5/cp/notification-template-wrapper.png')}
      alt="Template Wrapper setting inside form builder"
      scale="80"
    />
  </TabItem>
  <TabItem value="template-wrappers-template" label="Global Template">
    If you're creating/editing template files directly, you will need to add a line for `wrapperId` with the ID of the template wrapper in the heading comment block of code:
...
<Tabs>
  <TabItem value="predefined-assets-form" label="Form Builder" default>
    <Photo
      img={require('@site/static/img/craft/freeform/v5/cp/notification-template-assets.png')}
      alt="Predefined Assets setting inside form builder"
      scale="80"
    />
  </TabItem>
  <TabItem value="predefined-assets-template" label="Global Template">
...
<Tabs>
  <TabItem value="pdf-templates-form" label="Form Builder" default>
    <Photo
      img={require('@site/static/img/craft/freeform/v5/cp/notification-template-pdf.png')}
      alt="PDF Template setting inside form builder"
      scale="80"
    />
  </TabItem>
  <TabItem value="pdf-templates-template" label="Global Template">
...
<Photo
  img={require('@site/static/img/craft/freeform/v5/configuration/template-manager-notification-method.jpg')}
  alt="Email Notification Template Method setting in Template Manager"
/>
...
- May also use `allFields` or `allFieldsAndBlocks` variables to automate parsing of fields.
  ...
  - To parse only fields with data, you can use:
    ```twig
    {% for field in allFields|filter(field => field.valueAsString) %}
    ```
