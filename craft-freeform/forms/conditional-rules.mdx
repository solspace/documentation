...
sidebar_position: 5
description: Set up powerful conditional rules in Freeform 5.x for Craft CMS to show/hide fields, pages, and control form behavior based on user input.
---
...
{{ company.renderContainerOpeningTag({ attributes: { container: { class: "form-field" } } }) }}
    {{ company.renderLabel() }}
    {{ company.renderInput() }}
    {{ company.renderErrors() }}
{{ company.renderContainerClosingTag }}

{{ email.renderContainerOpeningTag({ attributes: { container: { class: "form-field" } } }) }}
