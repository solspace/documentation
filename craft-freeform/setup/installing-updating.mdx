...
<Tabs>
<TabItem value="install-cli" label="Command Line (CLI)" default>

### Install from the CLI<Badge type="recommended" text="Recommended" />

Freeform can be installed by running the following set of commands in your favorite CLI app:
...
<Tabs>
<TabItem value="install-cli-install-plugin-cli" label="CLI" default>
```sh
...
<Tabs>
<TabItem value="update-cli" label="Command Line (CLI)" default>
...
Check for available migrations for Freeform only:
