...
<Tabs>
  ...
  <TabItem value="static-caching-blitz-csrf" label="Blitz `csrfInput()`">
    ...
    Another option is to use [Blitz's CSRF Token Input tag](https://putyourlightson.com/plugins/blitz#craft-blitz-csrfinput). However, this method requires coding your form formatting inside your template rather than using a [formatting template](../../templates/formatting/index.mdx), which can be quite inconvenient.
