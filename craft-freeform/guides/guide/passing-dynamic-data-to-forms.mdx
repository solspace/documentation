---
sidebar_position: 1
description: Pass dynamic data (from query strings, entries, etc.) to Freeform 5.x forms in Craft CMS.
---
...
:::info
Freeform automatically captures the submitting user when the form is submitted by a logged-in user. This is available via `author` on the [Submission object](../../templates/objects/submission.mdx#author), with access to user properties such as `submission.author.id` and `submission.author.fullName`.
:::
...
:::info
Freeform automatically captures the full page URL where each form submission originates. This value is available via `sourceUrl` on the [Submission object](../../templates/objects/submission.mdx#sourceurl).<Badge type="feature" text="New in 5.15+" />
:::
...
:::info
The new [URL Parameter Tracking](../../integrations/url-parameter-tracking.mdx) integration can handle this automatically for you!<Badge type="feature" text="New in 5.12+" />
:::
