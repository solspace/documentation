...
<details>
  ...
  <AnswerCard>
    Getting an error like `Table already exists...` while running migrations during an upgrade is almost always due to a previously failed migration that is trying to be run again.
...
<details>
  ...
  <AnswerCard>
    ...
    - **Form/template/website is cached** - if using cache, please see the [Caching Forms](./guide/caching-forms.mdx) documentation guide to see how you will need to manually refresh the Craft CSRF token.
