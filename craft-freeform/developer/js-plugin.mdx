...
description: JavaScript plugin reference for Freeform 5.x in Craft CMS, covering form events, plugin options, manual script loading, and extensibility hooks for AJAX, Stripe, table fields, and more.
---
...
- `freeform.loadFreeformPluginJs()`<Badge type="feature" text="New in 5.14+" /> - will load Freeform's global `freeform.js` file.
- `freeform.loadFreeformPluginCss()`<Badge type="feature" text="New in 5.14+" /> - will load Freeform's global `freeform.css` file.
...
### `loadFreeformPluginCss()`, `loadFreeformPluginJs()` and `loadFormSpecificScripts()` Example

<Tabs>
<TabItem value="ff-manual-cssjs-loadffplugin-example" label="Example Code" default>
...
<TabItem value="ff-manual-cssjs-loadffplugin-output" label="Output">
