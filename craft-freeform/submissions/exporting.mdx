---
sidebar_position: 2
description: Export submissions from Freeform 5.x in Craft CMS using Quick Export or advanced Export Profiles (Pro).
---
...
:::info
In Freeform 5.9+, _Quick Export_ became available to all editions of Freeform.
:::

:::tip
[Need to build your own custom Export Type? Check out the guide.](../developer/export-types.mdx)
:::

The _Quick Export_ feature displays inside the Submissions page of the Freeform control panel. Clicking the _Quick Export_ button at the top of the page allows you to export all submissions for a given form, in your choice of **Excel**, **CSV**, **JSON**, **XML** or **Text**. You can then exclude whichever fields you don't want exported.
...
<Photo
  img={require('@site/static/img/craft/freeform/v5/cp/submissions-quick-export.png')}
  alt="Quick Export"
/>

## Export Profiles<BadgeGroup><Badge type="lite" text="Lite" /><Badge type="pro" text="Pro" /></BadgeGroup>

:::info
In Freeform 5.9+, _Export Profiles_ became available to the _Lite_ edition of Freeform.
:::
...
The _Export Profiles_ feature allows admins to create more complex pre-defined export profiles that can be run by any user with proper permissions at any time. Options for export profiles are as follows:
...
<Photo
  img={require('@site/static/img/craft/freeform/v5/cp/export-profiles-create.png')}
  alt="Create a New Export Profile"
/>
...
<Photo
  img={require('@site/static/img/craft/freeform/v5/cp/export-profiles-list.png')}
  alt="Export Profiles List"
/>
...
- **Enabled**<Badge type="feature" text="New in 5.14+" />
  - Controls whether this notification is currently active.
  - Useful if you have some forms that are only active for periods of time.
- **Name**
...
- **Frequency**
  - Specify how often and which days (if applicable) you'd like to receive the email notifications. The chosen date will attempt to send a notification in the morning (around 6am):
    ...
    - **Weekly on Fridays**
    - **Weekly on Saturdays**
    - **Custom Cron Schedule**<Badge type="feature" text="New in 5.15.27+" />
      - **Cron Expression**
        - Enter a standard five-part cron expression (minute, hour, day of month, month, day of week).
        - For example, `0 0 1 * *` runs monthly on the 1st, `0 0 15 * *` runs monthly on the 15th, and `0 0 1,15 * *` runs on both.
        :::info
        For reliable delivery, configure your server to run `php craft freeform/export-notifications/run` routinely. Use [Crontab.guru](https://crontab.guru/) to help with building and validating expressions.
        :::
- **Emails to Notify**
...
<Photo
  img={require('@site/static/img/craft/freeform/v5/cp/export-profiles-notifications.png')}
