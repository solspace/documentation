...
# Vue.js<BadgeGroup><Badge type="lite" text="Lite" /><Badge type="pro" text="Pro" /></BadgeGroup><Badge type="feature" text="Improved in 5.16+" />
...
```html title="HeadlessContactForm.vue" showLineNumbers
<script setup lang="ts">
...
```html title="ContactFormWithSave.vue" showLineNumbers
<script setup lang="ts">
import { Freeform } from '@solspace/freeform-vue';
import { recommendedExtensions } from '@solspace/freeform-extensions';
...
For deep dives (behavior, testing cards, webhooks), view the [React JS](./reactjs.mdx#payments) docs.
