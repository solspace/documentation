...
Freeform’s Vue packages are client-side. In Nuxt, use a `.client.vue` component (or `<ClientOnly>`) and proxy `/freeform` — same pattern as [Next.js](./nextjs.mdx) for React. See the dedicated [Nuxt](./nuxt.mdx) guide.

## Example demos

- [Freeform Headless Vue Demo](https://github.com/solspace/freeform-headless-vue-demo) — Vite + Vue 3 app (REST + GraphQL tab)
- [Freeform Headless Nuxt Demo](https://github.com/solspace/freeform-headless-nuxt-demo) — Nuxt 3 + `@solspace/freeform-vue`
