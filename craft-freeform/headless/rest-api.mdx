...
:::info npm Public Beta — `@solspace/freeform-*` `0.1.0-beta.1`
The headless REST API ships with the Freeform plugin (normal release). Official npm clients are on a separate **`0.1.0-beta.x`** line.
:::
...
Merge into `config/freeform.php`:
...
| `saveDraft` | Reserved — drafts are not in this beta |
...
`pluginVersion` matches Freeform. `minimumClientVersion` matches the npm client line (`0.1.0-beta.x` satisfies `0.1.0+`).
