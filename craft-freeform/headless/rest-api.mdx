...
title: REST API
sidebar_position: 2
description: Freeform headless REST API for manifests, submissions, CSRF, CORS, file uploads, and payment routes in Craft CMS.
---
...
# REST API<BadgeGroup><Badge type="lite" text="Lite" /><Badge type="pro" text="Pro" /></BadgeGroup><Badge type="feature" text="New in 5.16+" />
...
Most customers should use [`@solspace/freeform-react`](./reactjs.mdx) or [`@solspace/freeform-vue`](./vuejs.mdx) and never call these endpoints by hand. This page is the contract reference when you build a custom client or debug requests.
...
<Tabs>
  <TabItem value="payments-stripe" label="Stripe" default>
    ...
    Official clients call this for you. Prefer [`@solspace/freeform-react`](./reactjs.mdx#payments) or [`@solspace/freeform-vue`](./vuejs.mdx#payments) rather than calling the endpoint by hand.
  ...
  <TabItem value="payments-square" label="Square">
    ...
    Official clients call this for you. Prefer [`@solspace/freeform-react`](./reactjs.mdx#payments) or [`@solspace/freeform-vue`](./vuejs.mdx#payments) rather than calling the endpoint by hand. This is the same Square charge route used by classic Freeform templates.
  </TabItem>
  <TabItem value="payments-paypal" label="PayPal">
    ...
    Official clients call these for you. Prefer [`@solspace/freeform-react`](./reactjs.mdx#payments) or [`@solspace/freeform-vue`](./vuejs.mdx#payments) rather than calling the endpoints by hand. These are the same PayPal routes used by classic Freeform templates.
  </TabItem>
  <TabItem value="payments-mollie" label="Mollie">
    ...
    Official clients call create for you. Prefer [`@solspace/freeform-react`](./reactjs.mdx#payments) or [`@solspace/freeform-vue`](./vuejs.mdx#payments) rather than calling the endpoints by hand. These are the same Mollie routes used by classic Freeform templates.
  </TabItem>
</Tabs>
...
- [React JS](./reactjs.mdx) — React `<Freeform />` and `useFreeform()`
- [Vue.js](./vuejs.mdx) — Vue `<Freeform />` and `useFreeform()`
- [Next.js](./nextjs.mdx) — App Router + proxy
- [Nuxt](./nuxt.mdx) — Nuxt + client-only Freeform + proxy
- [React JS → Stripe payments](./reactjs.mdx#payments) — Payment Element in headless React
- [React JS → Square payments](./reactjs.mdx#payments) — Web Payments SDK in headless React
- [React JS → PayPal payments](./reactjs.mdx#payments) — PayPal Buttons in headless React
- [React JS → Mollie payments](./reactjs.mdx#payments) — hosted checkout redirect in headless React
- [Vue.js → Payments](./vuejs.mdx#payments) — same payment extensions in Vue
