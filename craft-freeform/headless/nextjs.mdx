...
# Next.js<BadgeGroup><Badge type="lite" text="Lite" /><Badge type="pro" text="Pro" /></BadgeGroup><Badge type="feature" text="Improved in 5.16+" />
...
See [React JS](./reactjs.mdx) for themes (including the official Tailwind and Bootstrap starters), captchas, **Stripe**, **Square**, **PayPal**, and **Mollie** payments, custom renderers, and the full hook API.
...
<Tabs>
  <TabItem value="payments-stripe" label="Stripe" default>
    Stripe Payment Element forms work with the same Client Component + `/freeform` rewrite pattern. Pass `recommendedExtensions` and follow [React JS → Stripe payments](./reactjs.mdx#payments). Keep Stripe webhooks pointed at your Craft site.
  </TabItem>
  <TabItem value="payments-square" label="Square">
    Square Web Payments SDK forms work with the same Client Component + `/freeform` rewrite pattern. Pass `recommendedExtensions` and follow [React JS → Square payments](./reactjs.mdx#payments).
  </TabItem>
  <TabItem value="payments-paypal" label="PayPal">
    PayPal Buttons forms work with the same Client Component + `/freeform` rewrite pattern. Pass `recommendedExtensions` and follow [React JS → PayPal payments](./reactjs.mdx#payments).
  </TabItem>
  <TabItem value="payments-mollie" label="Mollie">
    Mollie hosted-checkout forms work with the same Client Component + `/freeform` rewrite pattern. Pass `recommendedExtensions` and follow [React JS → Mollie payments](./reactjs.mdx#payments). Mollie callback/webhook must reach your Craft site.
  </TabItem>
</Tabs>
