...
```css title="app.css"
@import "tailwindcss";
@source "../node_modules/@solspace/freeform-react-theme-tailwind";
```

```tsx
import { Freeform } from '@solspace/freeform-react';
import { tailwindTheme } from '@solspace/freeform-react-theme-tailwind';

<Freeform handle="contact" baseUrl="…" theme={tailwindTheme} />;
```

Use `tailwindDarkTheme` on a dark page. Do not import `@solspace/freeform-react-theme-default/styles.css` on the same form. Override slots with `createTheme({ classNames: { submitButton: "…" } })` from the Tailwind package.
