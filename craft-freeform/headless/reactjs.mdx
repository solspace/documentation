...
# React JS<BadgeGroup><Badge type="lite" text="Lite" /><Badge type="pro" text="Pro" /></BadgeGroup><Badge type="feature" text="Improved in 5.16+" />
...
## Render with `<Freeform />`<Badge type="recommended" text="Recommended" />
...
<Tabs>
  <TabItem value="payments-stripe" label="Stripe" default>
  ...
  <TabItem value="payments-square" label="Square">
  ...
  <TabItem value="payments-paypal" label="PayPal">
  ...
  <TabItem value="payments-mollie" label="Mollie">
    ...
    - Mollie returns through Freeform’s callback **on your app origin** (same-origin `/freeform` proxy) so Craft session cookies still match; the webhook updates the PaymentRecord on Craft.
...
## Same-origin Proxy<Badge type="recommended" text="Recommended" />
