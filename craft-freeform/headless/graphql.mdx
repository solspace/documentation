...
The official React packages already accept a custom `fetch`. Point Freeform REST URLs at GraphQL with a small adapter (see the [example demo](https://github.com/solspace/freeform-headless-react-demo) `graphqlFetch` helper):
...
The [Freeform Headless React Demo](https://github.com/solspace/freeform-headless-react-demo) **GraphQL** tab includes live `<Freeform />`, `useFreeform()`, and raw query examples. Set `VITE_GRAPHQL_TOKEN` (and optional `VITE_GRAPHQL_PATH`) in `.env`.
