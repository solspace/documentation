...
# GraphQL<BadgeGroup><Badge type="lite" text="Lite" /><Badge type="pro" text="Pro" /></BadgeGroup><Badge type="feature" text="Improved in 5.16+" />

Freeform supports GraphQL in two ways:

- **Headless Adapters**<Badge type="recommended" text="Recommended" /><Badge type="feature" text="New in 5.16+" />
    - `freeformHeadlessManifest` and `freeformHeadlessSubmit`, matching the [REST API](./rest-api.mdx) contract
- **Legacy Form Queries / `save_{handle}_Submission` mutations**
    - still supported for existing integrations ([documented below](#legacy-graphql-form-queries-and-submission-mutations))
...
:::tip New React / Vue / Next.js projects
...
## Headless GraphQL Adapters<Badge type="feature" text="New in 5.16+" />
...
Multipart / File Drag & Drop uploads remain on the **REST** endpoints. Use GraphQL for JSON values; upload files via REST (or the official React / Vue packages on REST).

### Try it with `<Freeform />` / `useFreeform()`

The official React and Vue packages already accept a custom `fetch`. Point Freeform REST URLs at GraphQL with a small adapter (see the [React demo](https://github.com/solspace/freeform-headless-react-demo) or [Vue demo](https://github.com/solspace/freeform-headless-vue-demo) `graphqlFetch` helper):

```tsx title="GraphQLFreeform.tsx" showLineNumbers
import { Freeform, useFreeform } from '@solspace/freeform-react';
import { recommendedExtensions } from '@solspace/freeform-extensions';
import { graphqlFetch } from './graphqlFetch'; // demo helper — copy from the example repo
...
- `GET .../manifest` → `freeformHeadlessManifest`
- `POST .../submit` (JSON) → `freeformHeadlessSubmit`
- Multipart file uploads + CSRF token fetch stay on REST

### Try it

The [Freeform Headless React Demo](https://github.com/solspace/freeform-headless-react-demo) and [Vue Demo](https://github.com/solspace/freeform-headless-vue-demo) **GraphQL** tabs include live `<Freeform />`, `useFreeform()`, and raw query examples. Set `VITE_GRAPHQL_TOKEN` (and optional `VITE_GRAPHQL_PATH`) in `.env`.
...
| `siteIds`<Badge type="feature" text="New in 5.13.6+" />     | `[Int]`    | Narrow the queried forms by one or more site IDs     |
| `siteHandles`<Badge type="feature" text="New in 5.13.6+" /> | `[String]` | Narrow the queried forms by one or more site handles |
...
mutation SaveContactSubmission(
    ...
    $urlParameterTracking: [FreeformUrlParameterTrackingInputType],
    ...
    save_contact_Submission(
        ...
        urlParameterTracking: $urlParameterTracking
        captcha: $captcha
        csrfToken: $csrfToken
        firstName: $firstName
        lastName: $lastName
...
// Query Variables
{
    ...
    "urlParameterTracking": [
        {"name": "utm_source", "value": "INSERT UTM SOURCE VALUE"},
        {"name": "utm_medium", "value": "INSERT UTM MEDIUM VALUE"},
        {"name": "utm_campaign", "value": "INSERT UTM CAMPAIGN VALUE"}
    ],
    "firstName": "Solspace",
    "lastName": "Freeform",
...
### Form Interface<Badge type="feature" text="Improved in 5.9.6+" />
...
| `captcha`<Badge type="deprecated" text="Deprecated in 5.9.7+" />     | [`FreeformCaptchaInterface`](#captcha-interface)                               | The captcha for this form                                              |
| `captchas`<Badge type="feature" text="Renamed in 5.9.7+" />          | [`[FreeformCaptchaInterface]`](#captcha-interface)                             | The captchas for this form                                             |
...
| `successBehavior`<Badge type="feature" text="New in 5.9.6+" />       | `String`                                                                       | The form's success behavior                                            |
| `successTemplate`<Badge type="feature" text="New in 5.9.6+" />       | `String`                                                                       | The form's success template                                            |
...
| `duplicateCheck`<Badge type="feature" text="New in 5.9.6+" />        | `String`                                                                       | The form's duplicate check rule                                        |
| `stopSubmissionsAfter`<Badge type="feature" text="New in 5.9.6+" />  | `String`                                                                       | The form's stop submissions after date                                 |
| `enctype`                                                            | `String`                                                                       | The form's enctype                                                     |
| `rules`                                                              | [`FreeformRulesInterface`](#rules-interface)                                   | The form's rules                                                       |
| `postForwarding`<Badge type="feature" text="New in 5.9.5+" />        | [`FreeformPostForwardingInterface`](#post-forwarding-interface)                | The Post Forwarding integration for this form                          |
| `gtm`<Badge type="feature" text="New in 5.9.5+" />                   | [`FreeformGoogleTagManagerInterface`](#google-tag-manager-gtm-interface)       | The Google Tag Manager integration for this form                       |
| `javascriptTest`<Badge type="feature" text="New in 5.9.6+" />        | [`FreeformJavascriptTestInterface`](#javascript-test-interface)                | The Javascript Test integration for this form                          |
| `urlParameterTracking`<Badge type="feature" text="New in 5.15.1+" /> | [`[FreeformUrlParameterTrackingInterface]`](#url-parameter-tracking-interface) | The URL Parameter Tracking integration for this form                   |
| `siteId`<Badge type="feature" text="New in 5.13.6+" />               | `Int`                                                                          | The form's site ID                                                     |
| `siteUid`<Badge type="feature" text="New in 5.13.6+" />              | `String`                                                                       | The form's site UID                                                    |
| `siteName`<Badge type="feature" text="New in 5.13.6+" />             | `String`                                                                       | The form's site name                                                   |
| `siteHandle`<Badge type="feature" text="New in 5.13.6+" />           | `String`                                                                       | The form's site handle                                                 |
...
| `requiredMessage`<Badge type="feature" text="New in 5.10.8+" /> | `String`                                                 | The custom required message for the field |
...
| `postForwarding`<Badge type="feature" text="New in 5.9.5+" />        | [`FreeformPostForwardingInterface`](#post-forwarding-interface)                | The Post Forwarding integration of the submission    |
| `gtm`<Badge type="feature" text="New in 5.9.5+" />                   | [`FreeformGoogleTagManagerInterface`](#google-tag-manager-gtm-interface)       | The Google Tag Manager integration of the submission |
| `javascriptTest`<Badge type="feature" text="New in 5.9.6+" />        | [`FreeformJavascriptTestInterface`](#javascript-test-interface)                | The Javascript Test integration of the submission    |
| `urlParameterTracking`<Badge type="feature" text="New in 5.15.1+" /> | [`[FreeformUrlParameterTrackingInterface]`](#url-parameter-tracking-interface) | The URL Parameter Tracking integration for this form |
...
| Field                                                          | Type     | Description                |
|----------------------------------------------------------------|----------|----------------------------|
| `errorMessage`<Badge type="feature" text="New in 5.9.6+" />    | `String` | Error message for Honeypot |
| `name`                                                         | `String` | Name of Honeypot           |
| `value`<Badge type="deprecated" text="Deprecated in 5.9.7+" /> | `String` | Value of Honeypot.         |
...
### Captcha Interface<Badge type="feature" text="Renamed in 5.0+, Improved in 5.9.6+" />

Use `FreeformCaptchaInterface`.

:::warning
In Freeform 5.9.6+, `FreeformSubmissionCaptchaInputType` was renamed to `FreeformCaptchaInputType`. If using `FreeformSubmissionCaptchaInputType`, be sure to change it to `FreeformCaptchaInputType`.
:::

| Field                                                            | Type      | Description                                        |
|------------------------------------------------------------------|-----------|----------------------------------------------------|
| `name`                                                           | `String`  | The forms GraphQL mutation name for submissions    |
| `handle`<Badge type="deprecated" text="Deprecated in 5.9.7+" />  | `String`  | The forms GraphQL mutation handle for submissions. |
| `enabled`<Badge type="deprecated" text="Deprecated in 5.9.7+" /> | `Boolean` | Is Captcha enabled for this form.                  |
| `action`<Badge type="feature" text="New in 5.9.6+" />            | `String`  | The Captcha action                                 |
| `errorMessage`<Badge type="feature" text="New in 5.9.6+" />      | `String`  | The Captcha custom error message                   |
| `failureBehavior`<Badge type="feature" text="New in 5.9.6+" />   | `String`  | The Captcha failure behaviour                      |
| `fieldHandle`<Badge type="feature" text="New in 5.9.6+" />       | `String`  | The Captcha field handle                           |
| `locale`<Badge type="feature" text="New in 5.9.6+" />            | `String`  | The Captcha locale                                 |
| `scoreThreshold`<Badge type="feature" text="New in 5.9.6+" />    | `String`  | The Captcha score threshold                        |
| `size`<Badge type="feature" text="New in 5.9.6+" />              | `String`  | The Captcha size                                   |
| `theme`<Badge type="feature" text="New in 5.9.6+" />             | `String`  | The Captcha theme                                  |
| `triggerOnInteract`<Badge type="feature" text="New in 5.9.6+" /> | `Boolean` | Will the Captcha trigger on interaction            |
| `version`<Badge type="feature" text="New in 5.9.6+" />           | `String`  | The Captcha version                                |
...
### Post Forwarding Interface<Badge type="feature" text="New in 5.9.5+" />
...
### Google Tag Manager (GTM) Interface<Badge type="feature" text="New in 5.9.5+" />
...
### Javascript Test Interface<Badge type="feature" text="New in 5.9.6+" />
...
### URL Parameter Tracking Interface<Badge type="feature" text="New in 5.15.1+" />

Use `FreeformUrlParameterTrackingInterface`.

| Field        | Type     | Description                                           |
|--------------|----------|-------------------------------------------------------|
| `parameters` | `String` | The list of URL paramter tracking names for this form |
...
| ----------------------------------------------------------------- | --------- | -------------------------------------------------------------------- |
| `value`                                                           | `String`  | The default value                                                    |
| `checkedByDefault`<Badge type="feature" text="New in 5.10.7+" />  | `Boolean` | Should the checkbox be checked by default                            |
| `checked`<Badge type="deprecated" text="Deprecated in 5.10.7+" /> | `Boolean` | This attribute does not work. Please use `checkedByDefault` instead. |
...
- A `honeypot` entry if you're using the [Honeypot](../forms/spam-protection.mdx#honeypot).
