...
sidebar_position: 1
description: Install and enable Freeform's official headless React packages for Craft CMS (npm public beta).
---
...
:::info npm Public Beta — `0.1.0-beta.1`
The official `@solspace/freeform-*` packages ship on the npm **`beta`** tag (`0.1.0-beta.x`). The Freeform **Craft plugin** (with the headless REST API) ships as a normal release on main — not as a plugin beta. Package APIs may still change before a stable `0.1.0` / `1.0.0`.
:::
...
3. Install the official npm packages (`@beta`)
...
| Freeform plugin | Build that includes headless REST (merged to main) |
| npm packages | **`0.1.0-beta.1`** (`@beta` tag) |
| React | 18 or 19 |
...
<VerticalStepWrapper>
<StepMarkdown stepTitle="Enable headless in Freeform">

Headless is **off by default**. Add a `headless` section to your Craft project’s `config/freeform.php` (merge with your existing config):
...
return [
    'headless' => [
        ...
        // Per-form exposure (keyed by form handle)
        'forms' => [
            'contact' => [
                'exposeManifest' => true,
                'allowSubmit' => true,
...
## What’s included in this npm beta
