...
# Nuxt<BadgeGroup><Badge type="lite" text="Lite" /><Badge type="pro" text="Pro" /></BadgeGroup><Badge type="feature" text="New in 5.16+" />
...
<VerticalStepWrapper>
  ...
  <StepMarkdown stepTitle="Create a client-only form component">
    ```html title="components/ContactForm.client.vue" showLineNumbers
    ...
    ```html title="pages/contact.vue" showLineNumbers
    <template>
      <main>
        <h1>Contact</h1>
        <ClientOnly>
...
Stripe, Square, PayPal, and Mollie work with the same client component + `/freeform` proxy pattern. Pass `recommendedExtensions` and follow [Vue.js → Payments](./vuejs.mdx#payments).
