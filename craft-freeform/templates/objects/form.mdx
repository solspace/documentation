---
sidebar_position: 1
description: Form object reference and properties available in Freeform 5.x templates for Craft CMS.
---
...
### `stopSubmissionsAfter`<Badge type="feature" text="New in 5.11.8+" />

Returns the date specified in the **Stop Submissions After Date** setting for the form. Can be used as a conditional to hide the form and/or show a message to the user if the form has passed its close date.

```twig
{% if form.stopSubmissionsAfter|date >= now|date %}
  <p>The deadline to submit this form was {{ form.stopSubmissionsAfter|date }}.</p>
{% endif %}
```
