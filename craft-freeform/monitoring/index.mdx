...
sidebar_position: 1
description: Use Freeform 5.x Form Monitor for queue pinging, uptime checks, and improved reliability in Craft CMS.
---
...
# Form Monitoring Service<Badge type="pro" text="Pro" /><Badge type="feature" text="New in 5.11+" />
...
<FlexCards
  ...
  items={[
    ...
    },
    {
      ...
      description: 'Troubleshooting guides to common problems encountered.',
...
:::info
Please see the [Troubleshooting](#troubleshooting) guide further below for tips on resolving common issues.
:::
...
<VerticalStepWrapper>
  <StepMarkdown>
    ...
    - Find _Form Monitor_ in the list of **service providers**.
...
## Troubleshooting
If you're experiencing any issues with Form Monitor, please consider some of the following guides. If the issue persists, please [contact Support](/support/) for assistance.

### Cloudflare / WAF Allowlisting
If your site is protected by [Cloudflare](https://cloudflare.com) (or another WAF / bot protection service), _Form Monitor_ may be shown an interstitial like `Just a moment...`, `Checking your browser...` or an interactive challenge (e.g. [Turnstile](https://www.cloudflare.com/application-services/products/turnstile/)). If that challenge cannot be completed automatically, monitoring tests may fail even though the form works for real users.

To make monitoring reliable, we recommend _allowlisting_ Form Monitor's outbound IP address(es) in your security provider and skipping challenge actions for that allowlisted traffic.

#### What to Allowlist

- Allowlist the **Form Monitor outbound IP address(es)**.
  - Contact Solspace support to get the **current outbound IP list** for the Form Monitor service.

#### What to Change in Cloudflare

- Create a **WAF / Security rule** to **skip** (or reduce) bot protections for the allowlisted IPs.
- Scope the rule as narrowly as possible:
  - Only the specific hostname(s) you want monitored.
  - Optionally only the page(s) that contain monitored forms (e.g. `/contact`, `/support`, etc.).

Once _allowlisted_, Form Monitor can access the real form page directly and your monitoring runs should no longer be blocked by Cloudflare interstitials/challenges.
