...
sidebar_position: 2
description: Relating Calendar events to other elements in Solspace Calendar 6.x for Craft CMS.
---
...
In the example below, we'll first get the user's username from the 2nd segment of the URL, populate a user profile page, and feed the user ID to a Calendar Events query to display related events (the key here, being the `relatedTo` parameter):
