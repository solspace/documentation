...
- A common use-case for tokens would be when you want to have a more secure URL for accessing and displaying submission data, or allowing users to [edit their own submission](./submissions.mdx) and not requiring any login, etc.
- Required if you wish to allow [editing of submissions on front end](./submissions.mdx#editing-submissions).<Badge type="pro" text="Pro" />
...
```html showLineNumbers{1-21,73-75}
